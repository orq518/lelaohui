...
public class AddCameraActivity extends Activity implements OnClickListener,
	...
	public void onClick(View v) {
		...
		case R.id.done://连接
			if (tag == 1) {
				Toast.makeText(AddCameraActivity.this, "锟斤拷锟斤拷锟斤拷丫锟斤拷锟斤拷锟斤拷锟斤拷锟阶刺�...", 0)
						.show();
			} else if (tag == 2) {
				Toast.makeText(AddCameraActivity.this, "锟斤拷锟斤拷锟斤拷锟接ｏ拷锟斤拷锟皆猴拷...", 0).show();
			} else {
				done();
			}

			break;
		case R.id.btn_searchCamera://搜索
			searchCamera();
			break;
	...
	public void callBackSearchResultData(int cameraType, String strMac,
		...
		Log.d("-d", "strMac ="+strMac+"= strMac ="+
				strName+"= strDeviceID =" +strDeviceID);
